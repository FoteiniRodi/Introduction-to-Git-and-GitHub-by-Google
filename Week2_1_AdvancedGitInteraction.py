...
1.1.1	Intro to Module 2: Using Git Locally
1.1.2	Skipping the Staging Area
1.1.3	Getting More Information About Our Changes
1.1.4	Deleting and Renaming Files
1.1.5	Advanced Git Cheat Sheet
1.1.6	Practice Quiz

MY SYNOPSIS
COMMAND     "commit -a"     stage and commit in 1 step
and         "commit -m -a"  stage and commit in 1 step. Also add the commit message on the command line
...
ofcourse with -m flag, we write the commit message on the command line without opening the file on a text editor
...
shows some stats about the changes in the commit
...
COMMAND "git add -p"
git will show us the change being added and ASKS US if we want to stage it or not.
...
So, here we specifu rules that tell Git to IGNORE certain files
...
git mv
correct. git mv won't give you any information on changes. Instead, it is used to move or rename a file or directory in Git.
